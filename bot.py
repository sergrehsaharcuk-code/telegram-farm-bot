...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "register":
        ...
        country_id = parts[0]
        operator = "_".join(parts[1:])
        await buy_with_operator(query, context, country_id, operator)
...
async def buy_with_operator(query, context, country_id, operator):
    """Покупает номер у выбранного оператора"""
    balance = farm.get_balance()
    if balance is None:
...
async def generate_qr(query, context, filename):
    ...
    try:
        ...
        if not session_string:
            await query.edit_message_text("❌ Не удалось найти session_string")
            return
        ...
        await query.message.reply_photo(
            ...
            caption=f"🔑 *QR-код для входа*\n\n"
                    f"📱 *Как войти:*\n"
                    f"• *Android:* Telegram → «Войти по QR-коду»\n"
...
async def export_all_accounts(query, context):
    ...
    try:
        ...
        with open(all_zip, 'rb') as f:
            await query.message.reply_document(document=f, filename="all_accounts.zip", caption=f"📦 Все аккаунты ({len(accounts)} шт.)")
        os.remove(all_zip)
        await back_to_menu(query, context)
    except Exception as e:
...
async def show_help(query, context):
    help_text = (
        ...
        "   • Или нажать «QR» → отправить QR-код\n\n"
