...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "sort_cheap":
        await show_countries_sorted(query, context, reverse=False)
    elif data == "sort_expensive":
        await show_countries_sorted(query, context, reverse=True)
...
async def auto_farm(query, context):
    ...
    # Сохраняем в context
    context.user_data['all_prices'] = prices

    keyboard = [
        [InlineKeyboardButton("📈 Сначала дешёвые", callback_data="sort_cheap")],
    ...
    await query.edit_message_text(
        f"💰 **Выберите сортировку:**\n\n{balance_text}",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode=ParseMode.MARKDOWN
    )


async def show_countries_sorted(query, context, reverse=False):
    """Показывает страны с сортировкой по цене"""
    prices = context.user_data.get('all_prices')
    if not prices:
        await query.edit_message_text("❌ Данные не найдены, попробуйте снова")
        return

    # Сортируем
    sorted_prices = sorted(prices, key=lambda x: x.get('price', 0), reverse=reverse)

    keyboard = []
    for item in sorted_prices[:15]:
        country_name = item.get('name', f"Страна {item.get('id', '?')}")
    ...
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="auto_farm")])

    balance = farm.get_balance()
    balance_text = f"💰 Баланс: {balance:.2f} руб" if balance else "💰 Баланс: неизвестен"

    await query.edit_message_text(
        f"💰 **Страны для регистрации:**\n\n{balance_text}",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode=ParseMode.MARKDOWN
    )
...
async def show_help(query, context):
    help_text = (
        ...
        "1️⃣ *Автоферма*\n"
        "   • Нажми «АВТОФЕРМА»\n"
        "   • Выбери сортировку (сначала дешёвые/дорогие)\n"
        "   • Выбери страну\n"
